...
# Blog: http://www.danielclasson.com
# Usage: Powershell.exe -ExecutionPolicy ByPass .\ConfigMgr-Import-Tool-v1.0.ps1 -SiteServer <SiteServer> -SiteCode <SiteCode> -LogPath <Path to write log files to> -CSVPath <Initial directory used when browsing for CSV files> -Container <Collection Folder>
################################################################################################################################################
...
[string]$Container = "", #Folder containing OSD collections.
[Parameter(Mandatory=$True)]
[string]$SiteCode = "", #Site Code.
[Parameter(Mandatory=$True)]
[string]$SiteServer = "", #FQDN of Primary Site server. REQUIRED
...
# Blog URL
$OpenURL = {[System.Diagnostics.Process]::Start("http://www.danielclasson.com")}
...
$BlogURL.Text = "www.danielclasson.com"
